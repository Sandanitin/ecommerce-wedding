...
const Orders = () => {
  ...
  const handleViewDetails = async (order) => {
    try {
      // Fetch full order details to include fields like user.phone and full address
      const response = await adminApi.orders.getById(order._id);
      const fullOrder = response?.data?.data || response?.data || order;
      setSelectedOrder(fullOrder);
      setIsModalOpen(true);
    } catch (error) {
      console.error("Failed to load order details:", error);
      // Fall back to showing the existing order data if fetch fails
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <p>Phone: {selectedOrder.contactPhone || selectedOrder.user?.phone || "N/A"}</p>
              ...
              <div>
                ...
                <p>
                  Address: {selectedOrder.shippingAddress?.street || "N/A"}, {selectedOrder.shippingAddress?.city || "N/A"}, {selectedOrder.shippingAddress?.state || "N/A"} - {selectedOrder.shippingAddress?.zipCode || "N/A"} {selectedOrder.shippingAddress?.country ? `(${selectedOrder.shippingAddress.country})` : ""}
                </p>
              </div>
            </div>
            <div className="mb-4">
